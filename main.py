...
def get_dates_of_available_tickets(tickets_bsobj: BeautifulSoup) -> list:
    ...
    """
    dates = list()
    for p in tickets_bsobj.findAll("p", {"class": "css-1nm99ps"}): #chakra-text css-1nm99ps
        dates.append(p.text)
        print(dates)
    return dates

def trys(bsdj):
    rcb_shop_div = bsdj.find('div', {'id': 'rcb-shop'})

    # Check if div is found
    if rcb_shop_div:
        print("Div Content Found. Parsing through it...")
        
        # Example 1: Extract all paragraphs inside the div
        for p in rcb_shop_div.find_all('p'):
            print("Paragraph:", p.text)
        
        # Example 2: Extract all links (anchor tags)
        for link in rcb_shop_div.find_all('a'):
            print("Link:", link.get('href'))
        
        # Example 3: Extract all images
        for img in rcb_shop_div.find_all('img'):
            print("Image Source:", img.get('src'))

    else:
        print("Div with id='rcb-shop' not found.")
...
while not tickets_available:
    ...
    print(tickets_bsobj)
    # dates = [p for p in tickets_bsobj.select('link')]
    # print(dates)
    trys(tickets_bsobj)
